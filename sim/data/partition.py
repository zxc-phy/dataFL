...
class Partitioner():
    ...
    def check_dataidx_map(cls, dataidx_map=None, labels=None, num_clients=10, num_classes=10):
        ...
        total_distribution = n_sample_per_label / np.sum(n_sample_per_label)
        discrepency = {}
        for idx, distrb in n_sample_per_class_per_client.items():
            distrb = np.array(distrb)
            distrb = distrb / np.sum(distrb)
            discrepency[idx] = np.linalg.norm(distrb - total_distribution)
        print("\n****** the dicrepancy between per client and total distribution")
        print(discrepency)
...
def main():
    ...
    parser.add_argument('-n', type=int, default=30, help='divide into n clients')
    parser.add_argument('--partition', type=str, default='dir', help='iid')
    parser.add_argument('--balance', type=bool, default=True, help='balanced or imbalanced')
    parser.add_argument('--alpha', type=float, default=0.1, help='the alpha of dirichlet distribution')
