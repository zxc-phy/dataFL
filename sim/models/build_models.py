...
dataset_num_classes = {'mnist': 10, 'fashionmnist': 10, 'cifar10': 10, 'cinic10': 10, 'cifar100': 100, 'imagenet32': 1000, 'svhn': 10, 'har': 6, 'animal': 10, 'ham': 7}

def check(model, dataset):
    if 'resnet' in model:
        # model filter 1 if resnet
        model_pattern1 = r'logistic|mlp|lenet5|cnn|vgg|wvgg|resnetgnii|resnetii|wrngn|wrnnsgn|wrn|resnetgn\d+$|resnetgn\d+x\d+$|resnet\d+$|resnet\d+x\d+$|wrn\d+$|wrn\d+x\d+$|resnet'
        model_result1 = re.findall(model_pattern1, model)
        assert model_result1 != []
        model = re.sub(r'\d+', '', model)
    # model filter 2
    model_pattern2 = r'logistic|mlp|lenet|cnn|vgg|wvgg|resnetii|resnetgnii|resnetgn|wrngn|wrnnsgn|wrn|alexnet|resnet|densenet'    
    ...
    if dataset == 'cinic10':
        ...
        dataset_pattern = r'mnist|cifar|imagenet32|svhn|fashionmnist|cifar100|har|animal|ham'
...
def build_model(model='lenet5', dataset='mnist'):
    ...
    module = importlib.import_module('.{}_{}'.format(check_result[0], check_result[1]), 'sim.models')
    # class or function 
    if model in ['logistic', 'mlp', 'cnnmnist', 'lenet5', 'cnncifar', 'cnnsvhn', 'cnnhar', 'cnnanimal', 'lenetsvhn', 'alexnetcifar', 'cnnham']:
    ...
    if model in ['logistic', 'mlp', 'cnnmnist', 'lenet5'] and dataset in ['mnist', 'fashionmnist']:
        model = model_class()
    elif model == 'cnnsvhn' and dataset == 'svhn':
        model = model_class()
    elif model == 'cnnhar' and dataset == 'har':
        model = model_class()
    elif model == 'cnnanimal' and dataset == 'animal':
        model = model_class()
    elif model == 'cnnham' and dataset == 'ham':
        model = model_class()
    elif model == 'resnet' and dataset == 'svhn':
        model = model_class()
    elif model == 'lenetsvhn' and dataset == 'svhn':
        model = model_class()
    elif model == 'densenet' and dataset == 'svhn':
        model = model_class()
    elif model == 'cnncifar' and dataset in ['cifar10', 'cifar100']:
        model = model_class(num_classes=num_classes)
    elif model == 'alexnetcifar' and dataset in ['cifar10', 'cifar100']:
        model = model_class()
    elif check_result[0] in ['vgg', 'wvgg', 'resnetii', 'resnetgnii', 'resnetgn', 'wrn', 'wrngn', 'wrnnsgn'] and dataset in ['cifar10', 'cifar100', 'cinic10']:
        model = model_class(num_cla_classes)
    return model
